...
func (s *Bridge) GetHealthFromClient(id int64, c *conn.Conn) {
	for {
		if _, status, err := c.GetHealthInfo(); err != nil {
			break
...
func (s *Bridge) verifyStatusError(c *conn.Conn) {
	c.Write([]byte(common.VERIFY_STATUS_EER))
}
...
func (s *Bridge) cliProcess(c *conn.Conn) {
	...
	//verify
	id, err := file.GetMysqlDb().GetIdByVerifyKey(string(buf), c.Conn.RemoteAddr().String(), version.GetVersion(), session.MyIp)
	if err != nil {
		logs.Info("Current client connection validation error, close this client:", c.Conn.RemoteAddr())
		s.verifyError(c)
		return
	} else {
		if session.ClientIsConnected(id) {
			logs.Error("%d 已连接，先下线再连接", id)
			s.verifyStatusError(c)
			return
		}
		s.verifySuccess(c)
	}
...
func (s *Bridge) DelClient(id int64) {
	if v, ok := s.Client.Load(id); ok {
		...
		s.Client.Delete(id)
		if file.GetMysqlDb().IsPubClient(id) {
			return
		}
...
//use different
func (s *Bridge) typeDeal(typeVal string, c *conn.Conn, id int64, vs string) {

	isPub := file.GetMysqlDb().IsPubClient(id)
	switch typeVal {
	...
	case common.WORK_CHAN:
		session.ClientConnecting(id, version.GetVersion())
		...
		client, err := file.GetMysqlDb().GetClient(id)
		if err == nil {
			(&services.ClientService{}).FullClientRealRateFlow(client, true)
		}
...
func (s *Bridge) SendLinkInfo(clientId int64, link *conn.Link, t *file.Tunnel) (target net.Conn, err error) {
	...
	for {
		...
		case common.WORK_STATUS:
			if b, err := c.GetShortContent(32); err != nil {
				...
				var str string
				client, err := file.GetMysqlDb().GetClientIdByVkey(string(b))
				if err != nil {
					break loop
				}
				...
				file.GetDb().JsonDb.Tasks.Range(func(key, value interface{}) bool {
					...
					if _, ok := s.runList.Load(v.Id); ok && v.Client.Id == client.Id {
						str += v.Remark + common.CONN_DATA_SEQ
					}
					return true
				})
		...
		case common.NEW_CONF:
			...
			if client, err = c.GetConfigInfo(); err != nil {
				...
				if err = file.GetMysqlDb().NewClient(client); err != nil {
					fail = true
					c.WriteAddFail()
					break loop
				}
		...
		case common.NEW_HOST:
			...
			if !file.GetMysqlDb().HasHost(h, client) {
				if file.GetMysqlDb().IsHostExist(h) {
					fail = true
					c.WriteAddFail()
					break loop
				} else {
					file.GetMysqlDb().NewHost(h)
					c.WriteAddOk()
				}
			} else {
				c.WriteAddOk()
