...
func (s *JsonDb) LoadTaskFromJsonFile(client *sync.Map) {
	loadSyncMapFromFile(s.TaskFilePath, func(v string) {
		...
		if post.Client, err = s.GetClient(post.Client.Id, client); err != nil {
			return
		}
...
//		if post.Id > int64(s.HostIncreaseId) {
//			s.HostIncreaseId = int32(post.Id)
//		}
//	})
//}

func (s *JsonDb) GetClient(id int64, clients *sync.Map) (c *Client, err error) {
