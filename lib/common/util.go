...
//int64
func GetInt64NoErrByStr(str string) int64 {
	i, _ := strconv.ParseInt(strings.TrimSpace(str), 0, 64)
	return i
}
...
func ChangeHostAndHeader(r *http.Request, host string, header string, addr string, addOrigin bool, oldMatchUrl string, newMatchUrl string) {
	if host != "" {
		r.Host = host
	}
	if header != "" {
	...
	if oldMatchUrl != "" {
		newUrl := strings.Replace(r.RequestURI, oldMatchUrl, newMatchUrl, 1)
		newPath := strings.Replace(r.URL.Path, oldMatchUrl, newMatchUrl, 1)
		r.RequestURI = newUrl
		r.URL.Path = newPath
	}
