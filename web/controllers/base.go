...
//去掉没有err返回值的int
func (s *BaseController) GetInt64NoErr(key string, def ...int64) int64 {
	strv := s.Ctx.Input.Query(key)
	if len(strv) == 0 && len(def) > 0 {
		return def[0]
	}
...
//组装ajax
func ajax(str string, status int, data interface{}) map[string]interface{} {
		...
		if id := s.GetIntNoErr("id"); id != 0 {
			...
			if strings.Contains(s.actionName, "h") {
				if v, ok := file.GetDb().JsonDb.Hosts.Load(id); ok {
					if v.(*file.Host).Client.Id == s.GetSession("clientId").(int64) {
						belong = true
					}
				}
			} else {
				if v, ok := file.GetDb().JsonDb.Tasks.Load(id); ok {
					if v.(*file.Tunnel).Client.Id == s.GetSession("clientId").(int64) {
						belong = true
					}
				}
			}
