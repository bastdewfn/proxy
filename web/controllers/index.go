...
func (s *IndexController) GetTunnel() {
	...
	clientId := s.GetInt64NoErr("client_id")
...
func (s *IndexController) Add() {
	if s.Ctx.Request.Method == "GET" {
		...
		var err error
		if t.Client, err = file.GetMysqlDb().GetClient(s.GetInt64NoErr("client_id")); err != nil {
			s.AjaxErr(err.Error())
		}
...
func (s *IndexController) Edit() {
	...
	if s.Ctx.Request.Method == "GET" {
		...
		if t, err := file.GetDb().GetTask(id); err != nil {
			...
			if client, err := file.GetMysqlDb().GetClient(s.GetInt64NoErr("client_id")); err != nil {
...
func (s *IndexController) HostList() {
	if s.Ctx.Request.Method == "GET" {
		...
		start, length := s.GetAjaxParams()
		clientId := s.GetInt64NoErr("client_id")
		list, cnt := file.GetMysqlDb().GetHostPage(start, length, clientId, s.getEscapeString("search"), s.getEscapeString("sort"), s.getEscapeString("order"))
		s.AjaxTable(list, cnt, cnt, nil)
	}
}
...
func (s *IndexController) GetHost() {
	if s.Ctx.Request.Method == "POST" {
		data := make(map[string]interface{})
		if h, err := file.GetMysqlDb().GetHostById(s.GetInt64NoErr("id")); err != nil {
			data["code"] = 0
		} else {
			data["data"] = h
			data["code"] = 1
...
func (s *IndexController) DelHost() {
	...
	if err := file.GetMysqlDb().DelHost(id, 0); err != nil {
		s.AjaxErr("delete error")
	}
	s.AjaxOk("delete success")
}
...
func (s *IndexController) AddHost() {
	if s.Ctx.Request.Method == "GET" {
		...
		var err error
		if h.Client, err = file.GetMysqlDb().GetClient(h.ClientId); err != nil {
			s.AjaxErr("add error the client can not be found")
		}
		if err := file.GetMysqlDb().NewHost(h); err != nil {
...
func (s *IndexController) EditHost() {
	...
	if s.Ctx.Request.Method == "GET" {
		s.Data["menu"] = "host"
		if h, err := file.GetMysqlDb().GetHostById(id); err != nil {
			s.error()
		} else {
			s.Data["h"] = h
		}
		...
		if h, err := file.GetMysqlDb().GetHostById(id); err != nil {
			...
			cache.GetRedis().Del(cache.HOST_URLMATCH_KEY + h.Scheme + "_" + h.Location).Result()
			if h.Host != s.getEscapeString("host") {
				...
				if file.GetMysqlDb().IsHostExist(tmpHost) {
			...
			h.ReplaceLocation = s.getEscapeString("ReplaceLocation")
