...
//new client
func NewRPClient(svraddr string, vKey string, bridgeConnType string, proxyUrl string, cnf *config.Config, disconnectTime int, webServerIp string) *TRPClient {
	return &TRPClient{
		svrAddr:        svraddr,
...
retry:
	...
	if err != nil {
		logs.Error("The connection server failed and will be reconnected in five seconds, error", err)
		time.Sleep(time.Second * 5)
		goto retry
	}
	if c == nil {
...
func (s *TRPClient) getServerIp() bool {
	if s.webServerIp == "" && s.svrAddr == "" {
		logs.Error("server地址和webServer地址全为空")
		os.Exit(0)
		return false
	}
	if s.webServerIp != "" {
		resp, err := http.Get(s.webServerIp + "api/getliveserver?vkey=" + common.Getverifyval(s.vKey))
		if err != nil {
			logs.Error("根据webServer获取连接服务失败")
			return false
		}

		body, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			logs.Error("发起请求获取注册服务失败", err)
			return false
		}
		var webresult WebServerResult
		err = json.Unmarshal(body, &webresult)
		defer resp.Body.Close()
		if err != nil {
			logs.Error("发起请求获取注册服务失败，解析JSON失败", err)
			return false
		}
		if webresult.Status != 1 || webresult.Data == "" {
			logs.Error("没有可用服务，", webresult.Msg)
			return false
		}
		logs.Info("获取到可连接服务," + webresult.Data)
		s.svrAddr = webresult.Data
		...
		if targetConn, err := net.DialTimeout(common.CONN_TCP, lk.Host, lk.Option.Timeout); err != nil {
			...
			for {
				if r, err := http.ReadRequest(bufio.NewReader(srcConn)); err != nil {
					...
					logs.Trace("http request, method %s, host %s, url %s, remote address %s", r.Method, lk.Host, r.URL.Path, r.RemoteAddr)
					r.Write(targetConn)
				}
			}
		}
