...
re:
	...
	//	}
	NewRPClient(cnf.CommonConfig.Server, cnf.CommonConfig.VKey, cnf.CommonConfig.Tp, cnf.CommonConfig.ProxyUrl, cnf, cnf.CommonConfig.DisconnectTime, cnf.CommonConfig.WebServer).Start()
	CloseLocalServer()
	goto re
...
func NewConn(tp string, vkey string, server string, connType string, proxyUrl string) (*conn.Conn, error) {
	...
	if err := c.WriteLenContent([]byte(common.Getverifyval(vkey))); err != nil {
		return nil, err
	}
	...
	} else if s == common.VERIFY_STATUS_EER {
		return nil, errors.New(fmt.Sprintf("客户端已连接上，请先下线 %s", vkey))
	}
