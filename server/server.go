...
import (
	...
	"dewfn.com/nps/server/proxy"
	"dewfn.com/nps/server/tool"
)
...
func DealBridgeTask() {
	for {
		...
		case id := <-Bridge.CloseClient:
			...
			file.GetMysqlDb().OffLineClient(id)
			session.RemoveClient(id)
			//if v, ok := services.GetClients().Load(id); ok {
			//	if v.(*file.Client).NoStore {
			//		file.GetMysqlDb().DelClient(id)
			//	} else {
			//		session.RemoveClient(id)
			//		file.GetMysqlDb().OffLineClient(id)
			//	}
...
func GetTunnel(start, length int, typeVal string, clientId int64, search string) ([]*file.Tunnel, int) {
...
//get client list
func GetClientList(start, length int, search, sort, order string, clientId int64) (list []*file.Client, cnt int64) {

	list, cnt = file.GetMysqlDb().GetClientPage(start, length, search, sort, order, clientId)
...
func dealClientData() {
	//services.GetClients().Range(func(key, value interface{}) bool {
	//	v := value.(*file.Client)
	...
	//	v.Flow.InletFlow = 0
	//	v.Flow.ExportFlow = 0
	//	services.GetHosts().Range(func(key, value interface{}) bool {
	//		h := value.(*file.Host)
	//		if h.Client.Id == v.Id {
...
func DelTunnelAndHostByClientId(clientId int64, justDelNoStore bool) {
	var ids []int
	file.GetDb().JsonDb.Tasks.Range(func(key, value interface{}) bool {
		v := value.(*file.Tunnel)
	...
	if !justDelNoStore {
		file.GetMysqlDb().DelHost(0, clientId)
	}
}
...
func RemoteAction(clientId int64, actions string) bool {
	clientSesssion, _ := session.GetServerByClient(clientId)
	...
	if !session.IsLocalServer(clientSesssion.ServerIp) {
		...
		//md5Key:=crypt.Md5(configKey+strconv.FormatInt(timeNowUnix,10))
		newUrl := "http://" + clientSesssion.ServerIp + ":" +
			beego.AppConfig.String("web_port") + beego.AppConfig.String("web_base_url") + "/auth/remoteaction"

		postParams := url.Values{}
		postParams.Add("id", strconv.FormatInt(clientId, 10))
		postParams.Add("actions", actions)
		rsp, err := http.PostForm(newUrl, postParams)
		if err != nil {
			logs.Error("转调服务远程操作异常"+clientSesssion.ServerIp, err)
		}
...
func GetDashboardData() map[string]interface{} {
	...
	data["hostCount"] = len(file.GetMysqlDb().GetHost())
	clientList := file.GetMysqlDb().GetClientList()
	data["clientCount"] = len(clientList)
	...
	for _, client := range clientList {
		...
		in += client.Flow.InletFlow
		out += client.Flow.ExportFlow
	}
